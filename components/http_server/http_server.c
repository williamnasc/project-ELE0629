 ...
 */
esp_err_t http_server_power_off(httpd_req_t *req)
{
	
	printf("DESLIGA SERVER!\n");
	esp_restart();
 ...
 * @return ESP_OK
 */
static esp_err_t http_server_get_local_data_readings_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG, "/localData.json requested");

	char localDataJSON[100];

	sprintf(localDataJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\",\"humidity\":\"%.1f\"}", 5.0, 8.0, 9.0);

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, localDataJSON, strlen(localDataJSON));
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		// register OTAupdate handler
		httpd_uri_t power_off_server = {
				.uri = "/powerOffServer",
				.method = HTTP_POST,
				.handler = http_server_power_off,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &power_off_server);
		...
		// register localData.json handler
		httpd_uri_t local_data_json = {
				.uri = "/localData.json",
				.method = HTTP_GET,
				.handler = http_server_get_local_data_readings_json_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &local_data_json);
