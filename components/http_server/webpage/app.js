...
function powerOffServer(){
    ...
    request.open('POST', "/powerOffServer");
...
/**
 * Sets the interval for getting the data from spiffs.
 */
function startWeatherLocalData()
{
	$.getJSON('/localData.json', function(data) {
		$("#temperature_reading2").text(data["temp"]);
		$("#humidity_reading2").text(data["humidity"]);
	});
}
